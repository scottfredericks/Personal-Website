      ...
      for (const [key, vec] of Object.entries(DIRS)) {
        ...
        if (!grid[tx][ty]) {
          ...
          if (Math.abs(vec.x) === 1 && Math.abs(vec.y) === 1) {
            const n1x = (this.x + vec.x + cols) % cols; // neighbor 1, same y
            const n1y = this.y;
            const n2x = this.x;
            const n2y = (this.y + vec.y + rows) % rows; // neighbor 2, same x
...
function flushRenderQueues() {
  ...
  // Use butt caps to prevent the browser's internal AA
  ...
  for (let cIdx = 0; cIdx < currentPalette.length; cIdx++) {
    ...
    // Manually draw a circle at the end of every segment.
    // This covers the "butt" seam and creates the round join
...
function findAndSpawnFill(crawlerList) {
  // Identify cells that are currently being moved into.
  ...
  for (let x = 0; x < size; x += step) {
    for (let i = 0; i < scanHeight; i++) {
      ...
      // Skip if this cell is currently "under construction"
